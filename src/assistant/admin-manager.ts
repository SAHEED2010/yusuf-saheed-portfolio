...
export async function runAdminManager(instruction: string) {
  ...
  if (command.action === "publish_project") {
    ...
    if (!directPublishAllowed()) return { ok: false, action: command.action, requiresConfirmation: true, message: "Direct publishing is disabled. Review this record in Admin / Content and publish it manually after confirmation." };
  ...
  if (command.action === "update_project") {
    ...
    return { ok: true, action: command.action, lifecycle: candidate.lifecycle, requiresConfirmation: Boolean(command.publish && !canPublish), record: candidate };
  }
  ...
  if (command.action === "create_project" && command.project) {
    ...
    const canPublish = Boolean(command.publish && directPublishAllowed());
    const candidate = createProject({ slug: project.slug, title: project.title, summary: project.summary, role: project.role, body: Array.isArray(project.body) ? project.body.filter((value): value is string => typeof value === "string") : [], tags: Array.isArray(project.tags) ? project.tags.filter((value): value is string => typeof value === "string") : [], templateData: project.templateData as ProjectData, linkUrl: typeof project.linkUrl === "string" ? project.linkUrl : undefined, evidenceLabel: typeof project.evidenceLabel === "string" ? project.evidenceLabel : undefined, evidenceUrl: typeof project.evidenceUrl === "string" ? project.evidenceUrl : undefined, publish: canPublish });
    const errors = validateProject(candidate);
    if (errors.length) return { ok: false, error: errors.join("; ") };
