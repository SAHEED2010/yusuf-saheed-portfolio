...
export function VisitorAssistant({ aiLive = false }: { aiLive?: boolean }) {
  ...
  function ask(value: string, custom = false) {
    ...
    fetch("/api/assistant", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ question: value, custom }) })
      .then(async (response) => { const nextAnswer = await response.json() as VisitorAnswer; if (custom && response.ok) { const next = Math.max(0, remaining - 1); setRemaining(next); sessionStorage.setItem(allowanceStorageKey(), String(next)); } return nextAnswer; })
