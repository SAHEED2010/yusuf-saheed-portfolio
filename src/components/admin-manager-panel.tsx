...
export function AdminManagerPanel({ provider, model, ready }: { provider: string; model: string; ready: boolean }) {
  ...
  return <section className="admin-manager"><div className="admin-manager-head"><div><p className="eyebrow">Admin manager</p><h2>{ready ? `${provider} / ${model}` : "AI provider not configured"}</h2></div><span className="meta">{ready ? "connected" : "setup required"}</span></div><form className="admin-manager-form" onSubmit={submit}><label htmlFor="admin-instruction">Instruction<textarea id="admin-instruction" value={instruction} onChange={(event) => setInstruction(event.target.value)} placeholder="Prepare a hero-summary update or create a research draft…" maxLength={4000} required /></label><button type="submit" disabled={busy || !ready}>{busy ? "Working…" : "Prepare change"}</button></form><pre className="admin-manager-result" aria-live="polite">{result}</pre></section>;
}
