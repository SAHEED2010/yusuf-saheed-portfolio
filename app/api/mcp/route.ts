...
async function handle(request: Request) {
  ...
  if (origin && allowedOrigin) response.headers.set("Access-Control-Allow-Origin", origin);
...
export async function OPTIONS(request: Request) {
  ...
  const headers = new Headers({ "Access-Control-Allow-Headers": "Authorization, Content-Type, MCP-Protocol-Version, MCP-Session-Id, Last-Event-ID", "Access-Control-Allow-Methods": "GET, POST, DELETE, OPTIONS" });
